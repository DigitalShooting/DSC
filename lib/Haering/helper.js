...
module.exports = {
	getShotFromData: function(data, disziplin, session){
		...
		if (data[2] == "1d") {
			...
			var shot = this.getShotFromXY(x, y, scheibe, part);
			return shot;
		}
		else {
			return null;
	...
	getShotFromXY: function(x, y, scheibe, part){
		var shot = {
			ring: {
				display: 0,
				value: 0,
		...
		var ring = this.getRingFromTeiler(shot.teiler, scheibe);
		shot.ring.display = parseFloat(ring).toFixed(1);
		shot.ring.int = parseFloat(ring).toFixedDown(0);

		if (part.zehntel == true) {
			shot.ring.value = parseFloat(ring).toFixedDown(1);
		}
		else {
			shot.ring.value = parseFloat(ring).toFixedDown(0);
		}
