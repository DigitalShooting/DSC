...
module.exports = function(interf, disziplin){
	...
	esa.band = function(){
		exec([__dirname+'/API/bin/Band', interf.com], function(err, out, code) {
	...
	esa.nop = function(){
		exec([__dirname+'/API/bin/NOP', interf.com], function(err, out, code) {
	...
	esa.set = function(){
		exec([__dirname+'/API/bin/Set', interf.com], function(err, out, code) {

			exec([__dirname+'/API/bin/ReadSettings', interf.com], function(err, out, code) {
				var clean = cleanOutput(out)
				esa.onNewData(clean)
			});
		});
	...
	var intervalId
	setTimeout(function(){
		intervalId = setInterval(esa.nop, 750)
	}, 2000)
