...
module.exports = function(interf, disziplin){
	...
	esa.band = function(){
		child_process.exec([__dirname+'/API/bin/Band', interf.com], function(err, out, code) {
	...
	esa.nop = function(){
		child_process.exec([__dirname+'/API/bin/NOP', interf.com], function(err, out, code) {
			var shotData = cleanOutput(out)

			var shot = esaHelper.getShotFromData(shotData, disziplin)
			if (shot) {
				esa.onNewShot(shot)
	...
	esa.set = function(){
		child_process.exec([__dirname+'/API/bin/Set', interf.com], function(err, out, code) {

			child_process.exec([__dirname+'/API/bin/ReadSettings', interf.com], function(err, out, code) {
				var clean = cleanOutput(out)
				esa.onNewData(clean)
			});
		});
