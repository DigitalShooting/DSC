...
module.exports = function(interf, disziplin){
	...
	esa.band = function(){
		exec([__dirname+"/API/bin/HaeringAPI", interf.com, "band", disziplin.scheibe.band.onChangePart], function(err, out, code) {
	...
	esa.nop = function(){
		exec([__dirname+"/API/bin/HaeringAPI", interf.com, "nop"], function(err, out, code) {
	...
	esa.set = function(){
		exec([__dirname+"/API/bin/HaeringAPI", interf.com, "set", disziplin.scheibe.band.onShot], function(err, out, code) {
			exec([__dirname+"/API/bin/HaeringAPI", interf.com, "readSettings"], function(err, out, code) {
				var clean = cleanOutput(out)
				esa.onNewData(clean)
			});
		});
