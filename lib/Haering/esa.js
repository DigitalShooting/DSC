...
module.exports = function(interf, disziplin){
	...
	esa.band = function(){
		exec([__dirname+"/API/bin/Band", interf.com, "band", disziplin.scheibe.band.onChangePart], function(err, out, code) {
	...
	esa.nop = function(){
		exec([__dirname+"/API/bin/NOP", interf.com, "nop"], function(err, out, code) {
	...
	esa.set = function(){
		exec([__dirname+"/API/bin/Set", interf.com, "set", disziplin.scheibe.band.onShot], function(err, out, code) {

			exec([__dirname+"/API/bin/ReadSettings", interf.com, "readSettings"], function(err, out, code) {
				var clean = cleanOutput(out)
				esa.onNewData(clean)
			});
		});
