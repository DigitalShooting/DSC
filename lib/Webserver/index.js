...
var io_old = require("socket.io")(server);
...
// socket stuff
io_old.on('connection', function(socket){
...
wss.on('connection', function connection(ws) {
  ...
  ws.on("message", function(event){
    if (event == null) {
      console.log("Got null event", event);
      return;
    }
    ...
    else if (message.type == "newTarget") {
      checkAuth(message.data.auth, function(){
        process.send({type: "newTarget"});
      });
    }
    else if (message.type == "setDisziplin") {
      checkAuth(message.data.auth, function(){
        var key = message.data.disziplin;
        process.send({type: "setDisziplin", data: config.disziplinen.all[key]});
      });
    }
    else if (message.type == "setSelectedSerie") {
      checkAuth(message.data.auth, function(){
        process.send({type: "setSelectedSerie", data: message.data.index});
      });
    }
    else if (message.type == "setSelectedShot") {
      checkAuth(message.data.auth, function(){
        process.send({type: "setSelectedShot", data: message.data.index});
      });
    }
    else if (message.type == "setUser") {
      checkAuth(message.data.auth, function(){
        process.send({type: "setUser", data: message.data.user});
      });
    }
    else if (message.type == "setPart") {
      checkAuth(message.data.auth, function(){
        process.send({type: "setPart", data: {
          partId: message.data.partId,
          force: message.data.force,
        }});
      });
    }
    else if (message.type == "setSessionIndex") {
      checkAuth(message.data.auth, function(){
        process.send({type: "setSessionIndex", data: message.data.sessionIndex});
      });
    }
    else if (message.type == "print") {
      checkAuth(message.data.auth, function(){
        process.send({type: "print", data: message.data.printTemplate});
      });
    }
    else if (message.type == "loadData") {
      checkAuth(message.data.auth, function(){
        process.send({type: "loadData", data: message.data.data});
      });
    }
    else if (message.type == "getTempToken") {
      checkAuth(message.data.auth, function(){
        ws.send(JSON.stringify({type: "setTempToken", data: config.auth.tempKey}));
      });
    }
    ...
    else if (message.type == "sendSessions") {
      checkAuth(message.data.auth, function(){
        process.send({type: "sendSessions", data: {
