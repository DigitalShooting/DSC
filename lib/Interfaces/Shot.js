...
function roundToFixed(number, digits) {
  ...
  return parseFloat((Math.round(number * factor) / factor).toFixed(digits));
}
...
class Shot {
  ...
  constructor(x, y, scheibe, part) {
    ...
    this.teiler = roundToFixed((Math.pow((x*x + y*y), 0.5) / 10), 1).toFixed(1);
    ...
    this.winkel = roundToFixed(this.winkel, 1).toFixed(1);


    var ring = this.getRingFromTeiler(this.teiler, scheibe);
    this.ring.display = parseFloat(ring).toFixed(1);
    this.ring.int = roundToFixed(parseFloat(ring), 0);

    if (part.zehntel === true) {
      this.ring.value = roundToFixed(parseFloat(ring), 1);
    }
    else {
      this.ring.value = roundToFixed(parseFloat(ring), 0);
    }
  }
  ...
  getRingFromTeiler(teiler, scheibe){
    ...
    if (teiler == 0){
      ...
      ring = roundToFixed((m * teiler + t), 1).toFixed(1);
    }
    return ring;
  }
