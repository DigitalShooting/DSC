 ...
 */
class DeviceInterfaceRedDot extends DeviceInterface {
  ...
  constructor(interf, disziplin) {
    ...
    this.targetMode = disziplin.scheibe.interface.reddot.targetMode;
  ...
  initChild(interf, disziplin){
    ...
    this.child.stderr.on('data', data => {
      this.call("errorMessage", "RedDotAPI " + data.toString());
      this.call("onNewStatus", false);
      this.lostConnection = true;
    });
  ...
  checkRecovery() {
    if (this.lostConnection == true) {
      this.lostConnection = false;
      setTimeout(() => this.setMode(this.targetMode), 100);
    }
  }
...
module.exports = DeviceInterfaceRedDot;
