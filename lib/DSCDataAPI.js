...
module.exports = function(){
	...
	function initInterface(disziplin) {
		...
		activeInterface.on("message", function(text){
			api.on({ type: "message",  text: text});
		});
		activeInterface.on("errorMessage", function(text){
			api.on({ type: "message",  text: text});
		});
	...
	var api = {
		...
		setDisziplin: function(disziplin){
			api.on({ type: "message",  text: "[INFO] setDisziplin ("+disziplin.title+")"});
		...
		setPart: function(partId, force){
			api.on({ type: "message",  text: "[INFO] setPart ("+partId+")"});
		...
		setSessionIndex: function(sessionIndex, force){
			api.on({ type: "message",  text: "[INFO] setSessionIndex ("+sessionIndex+")"});
		...
		setUser: function(user){
			api.on({ type: "message",  text: "[INFO] setUser ("+JSON.stringify(user)+")"});
		...
		newShot: function(shot, sessionID, number){
			api.on({ type: "message",  text: "[INFO] newShot ("+shot.log()+")"});
			...
			// ################ GAME ################
			var game_new = false;
			if (part.mode != null && part.mode == "game_stairs") {
				if (session.currentLevel == null || session.gameDone == null) {
					session.currentLevel = 0;
					session.gameDone = false;
				}
				var step = part.steps[session.currentLevel];

				var last = step.minimum;
				if (serie != null && serie.shots.length >= 1) {
					last = serie.shots[serie.shots.length-1].ring.value;
				}
				console.log(last, step.minimum);


				if (session.gameDone == true) {
					return;
				}


				if (last < step.minimum) {
					game_new = true;
					// reset, target missed
					console.log("reset, target missed");
				}
				else if (serie != null && serie.anzahl >= step.repeats && part.steps.length - 1 <= session.currentLevel) {
					// game_new = true;
				...
				else if (serie != null && serie.anzahl == step.repeats) {
					game_new = true;
			...
			console.log("Current Level:", session.currentLevel+1, "/", part.steps.length);
			// ################################################
			...
			if (serie == null || part.serienLength == serie.anzahl || game_new) {
		...
		newTarget: function(){
			...
			api.on({ type: "message",  text: "[INFO] newTarget"});
		...
		setData: function(data){
			api.on({ type: "message",  text: "[INFO] setData"});
		...
		// get activeInterface, for testing purpos only
