...
module.exports = function(){
			...
			mysql.query("INSERT INTO session (sessionGroupID, part, date) VALUES(?, ?, ?);",
				...
				function(err, result) {
					...
					if (callbackDone != undefined){
						callbackDone(sqlVars.sessionID);
					}
				}
			);
		}
	}
	function sqlMakeNewShot(shot, session, sessionID){
		if (config.database.enabled){
			mysql.query("INSERT INTO shot (ring, ringValue, teiler, x, y, winkel, date, number, serie, sessionID) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
				[shot.ring.display, shot.ring.value, shot.teiler, shot.x, shot.y, shot.winkel, shot.time, shot.number, session.serien.length-1, sessionID],
	...
	var api = {
		...
		newShot: function(shot, sessionID, number){
			...
			sqlMakeNewShot(shot, session, sessionID == undefined ? sqlVars.sessionID : sessionID);
		},
