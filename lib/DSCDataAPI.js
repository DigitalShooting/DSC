...
module.exports = function(){
	...
	function updateData(){
		if (activeData !== undefined && collection !== undefined){
			collection.update({"_id" : activeData._id}, activeData, {upsert: true}, function(err, results) { });
		}
	}
	...
	var api = {
		...
		setPart: function(partId, force){
			...
			if (partId != activeSession.type || force === true){
				...
				if (exitType == "beforeFirst" && force === false){
					anzahlShots = activeSession.disziplin.parts[activeSession.type].anzahlShots;
					...
					anzahlShots = activeSession.serien.length < activeSession.disziplin.parts[activeSession.type].anzahlShots;
		...
		newShot: function(shot, sessionID, number){
			...
			shot.number = number === undefined ? session.anzahl+1 : number;
		...
		setData: function(data){
			...
			// set new disziplin
			activeDisziplin = lastObject(activeData.sessionParts).disziplin;

			// set up new interface
			activeInterface = config.interface[activeDisziplin.interface.name];
			activeInterface = require("../"+activeInterface.path)(activeInterface, activeDisziplin);
			activeInterface.onNewShot = function(shot){
				api.newShot(shot);
			};
			activeInterface.onNewData = function(data){
