...
module.exports = () => {
	...
	function initInterface(disziplin) {
		...
		activeInterface.on("onNewShot", shot => api.newShot(shot));
		activeInterface.on("onNewData", data => {
			// console.log(data)
		});
		activeInterface.on("onNewStatus", connected => {
			api.on({
				type: "statusChanged",
				connected: connected,
			});
		});
		activeInterface.on("message", text => {
			api.on({ type: "message",  text: text});
		});
		activeInterface.on("errorMessage", text =>{
			api.on({ type: "message",  text: text});
		});
	...
	var api = {
		...
		setSessionIndex: (sessionIndex, force) => {
		...
		newShot: (shot, sessionID, number) => {
			...
			if (overtime == false) {
				...
				serie.anzahlInTime++;
				serie.schnitt = (Math.round(serie.gesamt/ serie.anzahlInTime * 10)/ 10).toFixed(1);
			}
			...
			if (overtime == false) {
				...
				session.anzahlInTime++;
				session.schnitt = (Math.round(session.gesamt/ session.anzahlInTime * 10)/ 10).toFixed(1);
			}
			...
			if (part.average.enabled === true){
				var hochrechnung = session.gesamt/ session.anzahlInTime * part.average.anzahl;
	...
	function setDataHelper(session){
		api.setPart(session.part, true, sessionID => {
