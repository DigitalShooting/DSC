...
module.exports = function(){
	...
	function sqlMakeNewShot(shot){
		...
		mysql.query("INSERT INTO shot (ring, teiler, x, y, winkel, date, number, sessionID) VALUES(?, ?, ?, ?, ?, ?, ?, ?);",
			[shot.ring.value, shot.teiler, shot.x, shot.y, shot.winkel, shot.time, shot.number, sqlVars.sessionID],
		...
		setPart: function(partId){
			...
			if (partId != activeSession.type){
				...
				if (exitType == "beforeFirst"){
					...
					console.log(activeSession.anzahl != 0, anzahlShots != 0, activeSession.anzahl < anzahlShots, anzahlShots)
